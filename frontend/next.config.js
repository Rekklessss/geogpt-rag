...
const nextConfig = {
  ...
  async rewrites() {
    ...
    const apiHost = process.env.NEXT_PUBLIC_API_HOST || '13.221.48.20'
  ...
  env: {
    NEXT_PUBLIC_API_BASE_URL: process.env.NODE_ENV === 'production' 
      ? `${process.env.NEXT_PUBLIC_API_PROTOCOL || 'https'}://${process.env.NEXT_PUBLIC_API_HOST || '13.221.48.20'}`
      : 'http://localhost',
  ...
  images: {
    domains: [
      'localhost', 
      process.env.NEXT_PUBLIC_API_HOST || '13.221.48.20'
    ],
