...
const nextConfig = {
  ...
  async rewrites() {
    ...
    const apiHost = process.env.NEXT_PUBLIC_API_HOST || '3.81.101.190'
  ...
  env: {
    NEXT_PUBLIC_API_BASE_URL: process.env.NODE_ENV === 'production' 
      ? `${process.env.NEXT_PUBLIC_API_PROTOCOL || 'https'}://${process.env.NEXT_PUBLIC_API_HOST || '3.81.101.190'}`
      : 'http://localhost',
  ...
  images: {
    domains: [
      'localhost', 
      process.env.NEXT_PUBLIC_API_HOST || '3.81.101.190'
    ],
